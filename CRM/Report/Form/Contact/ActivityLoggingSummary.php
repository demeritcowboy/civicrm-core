 ...
 */
class CRM_Report_Form_Contact_ActivityLoggingSummary extends CRM_Logging_ActivityReportSummary {
    ...
    $this->_columns = [
      'log_civicrm_entity' => [
        ...
        'fields' => [
          ...
          /*
           * @todo This doesn't work as-is because the temporary table
           * needs to know about it but it's in the base class hard-coded
           * to contact fields.
          'altered_activity_type' => [
            'default' => TRUE,
            'name' => 'activity_type_id',
            'title' => ts('Activity Type'),
            'alias' => 'modified_activity_civireport',
            'alter_display' => 'alterActivityTypeID',
          ],
           */
          'altered_contact' => [
            'default' => TRUE,
            'name' => 'subject',
            'title' => ts('Altered Activity'),
            'alias' => 'modified_activity_civireport',
          ],
          'altered_contact_id' => [
            'name' => 'id',
  ...
  public function from() {
    ...
    $clause = $clause ? "AND entity_log_civireport.entity_table = 'civicrm_activity'" : NULL;

    $joinClause = "
INNER JOIN civicrm_activity modified_activity_civireport
        ON (entity_log_civireport.{$detail['fk']} = modified_activity_civireport.id {$clause})";

    if (!empty($detail['joins'])) {
      $clause = $detail['entity_table'] ?? NULL;
      $clause = $clause ? "AND fk_table.entity_table = 'civicrm_activity'" : NULL;
      $joinClause = "
INNER JOIN `{$this->loggingDB}`.{$detail['joins']['table']} fk_table ON {$detail['joins']['join']}
INNER JOIN civicrm_activity modified_activity_civireport
        ON (fk_table.{$detail['fk']} = modified_activity_civireport.id {$clause})";
    }
...
LEFT  JOIN civicrm_contact altered_by_contact_civireport
  ...
  protected function addDetailReportLinksToRow($baseQueryCriteria, $row) {
    ...
    // @todo At the moment this returns blank because the detail report doesn't
    // exist.
    $url1 = CRM_Report_Utils_Report::getNextUrl('logging/activity/detail', "{$q}&snippet=4&section=2&layout=overlay", FALSE, TRUE);
    $url2 = CRM_Report_Utils_Report::getNextUrl('logging/activity/detail', "{$q}&section=2", FALSE, TRUE);
  ...
  protected function alterActivityTypeID($value, &$row, $selectedfield, $criteriaFieldName) {
    return CRM_Core_PseudoConstant::getLabel('CRM_Activity_BAO_Activity', 'activity_type_id', $value);
  }
