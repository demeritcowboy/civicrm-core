              ...
              <tbody>
                ...
                {if !$row.is_reserved }
                  <tr id="UFGroup-{$row.id}" data-action="setvalue" class="crm-entity {$row.class}{if NOT $row.is_active} disabled{/if}">
                    ...
                    <td class="crmf-description crm-editable" data-type="textarea">{if $row.description !== null}{$row.description|escape}{/if}</td>
                ...
                {foreach from=$rows item=row}
                  {if $row.is_reserved}
                    <tr id="UFGroup-{$row.id}" class="crm-entity{if !empty($row.class)} {$row.class}{/if}{if NOT $row.is_active} disabled{/if}">
                      ...
                      <td>{if $row.description !== null}{$row.description|escape}{/if}</td>
                      <td>{$row.group_type}</td>
                      <td>{$row.id}</td>
                      <td>{$row.module}</td>
                      <td>{$row.action|smarty:nodefaults|replace:'xx':$row.id}</td>
