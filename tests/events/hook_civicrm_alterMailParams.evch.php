...
return new class() extends EventCheck implements HookInterface {
  ...
  public function hook_civicrm_alterMailParams(&$params, $context = NULL): void {
    ...
    $this->assertRegularExpression('/^(messageTemplate|civimail|singleEmail|flexmailer)$/',
    ...
    foreach ($params as $key => $value) {
      ...
      if (isset($paramSpecs[$key]['regex']) && $value !== NULL) {
        $this->assertRegularExpression($paramSpecs[$key]['regex'], $value, "Parameter [$key => $value] should match regex ({$paramSpecs[$key]['regex']})");
      }
    }
